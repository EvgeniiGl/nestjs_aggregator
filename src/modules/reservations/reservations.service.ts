...
export class ReservationsService implements IReservation {
    ...
    async getReservations(filter: ReservationSearchOptions): Promise<Reservation[]> {
        const data = {...filter, user: filter.user.toString()}
        return await this.ReservationModel.find(data, {user: 0, _id: 0, __v: 0})
            .populate({path: 'hotel', select: {title: 1, description: 1, _id: 0}})
            .populate({path: 'hotelRoom', select: {images: 1, description: 1, _id: 0}})
            .exec()
    }
    ...
    async removeReservation(id: Types.ObjectId, user?: UserDocument): Promise<void> {
        ...
        if (!reservation) {
            throw new HttpException('Резервация не найдена', 400)
        }
        if (user && reservation.user != user._id) {
